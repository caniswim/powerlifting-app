...
citada não era declarada; a lista acima é o resultado da conciliação.

⚠️ **R185 SAIU DE NOVO, em 12/08/2026, e o motivo é diferente do da primeira vez.** Da
outra vez ele era declarado e não citado; agora a única citação dele — o mock meet "com
equipamento específico de competição", em §8 — foi **retirada por não existir na base**:
R185 não tem claim entre 04:04 e 05:05, e nenhuma claim do corpus afirma aquilo. Sem
citação legítima, a bijeção manda tirá-lo da declaração, e é o que está feito. **Sobra R7
como única procedência do canal para o simulado (§8)** — uma claim `PESSOAL`/`narrativa`,
não duas, e a nota de §8 registra a diferença que isso faz no argumento.
...
leitura **não baixa o TM** — segura a carga uma semana `[R1 @01:30]` `[R27 @05:38]`; duas
leituras consecutivas baixam por `K`; queda de ≥2 pontos corta a sessão `[R10 @00:34]`.

⚠️ **ENDEREÇO E MODO CORRIGIDOS (12/08/2026), e a correção rebaixa a regra.** Esta linha
citava `[R1 @01:04]`, que é o bloco de `V001-06`/`V001-29` — o limiar de dor, que não tem
nada a ver com não reagir a UMA leitura. A claim que o texto de fato usa é
**`V001-08`** `[R1 @01:30]`, *"um único treino mais doloroso não significa que o programa
de reabilitação não está funcionando"*, `[GERAL]` **`opiniao`**; e `[R27 @05:38]` é
**`V027-27`**, *"um dia isolado de pico de dor é normal"*, `[GERAL]` **`fato`**. Nenhuma
das duas é `prescricao`. **Portanto "não reagir à primeira leitura" é DECISÃO DE DESENHO**,
igual ao 1º degrau do §1.2 — que se apoia na mesma `V001-08`. As duas metades do documento
que amortecem o primeiro sinal estão ancoradas na mesma opinião, e isso agora está escrito
nos dois lugares em vez de em nenhum.
...
pausada com carga de trabalho · pós-sessão**, escala 0–10. **Isto inclui as 4 sessões de
peitoral da S17 e as 2 da S18** — o gate não relaxa quando os degraus acabam, porque no
taper ele deixa de congelar degrau e passa a **governar a 3ª tentativa de supino do
simulado** (§8), que é decidida com o log das duas semanas anteriores, ou seja, S17 e S18.

⚠️ **ENUMERAÇÃO CORRIGIDA (12/08/2026).** A versão anterior dizia *"as 4 sessões do taper
e o dia fácil da S18"*, o que deixava **o simulado de fora da conta** — e o simulado é
sessão de peitoral, é a mais pesada das duas da S18, e é justamente aquela cuja 3ª
tentativa este parágrafo diz que o gate governa. Medido no gerado: **S17 tem 4 sessões com
peitoral e S18 tem 2** (o dia fácil e o simulado).

**O floor press entra no gate como qualquer outra linha de peitoral**, e até esta revisão
ele era o único bloco de peitoral que o gate não conseguia reduzir.

⚠️ **NÚMERO CORRIGIDO (12/08/2026): não são "9 séries semanais".** A versão anterior
afirmava 9 e usava esse número como argumento de gravidade. Medido contra o gerador, o
floor press por semana é **9, 9, 9, 8, 7, 7, 6, 6, 5, 5, 5, 5, 5, 5, 5, 5** — a frase vale
em **3 das 16 semanas**, e da S9 em diante são 5. Nas S10–S11, que este mesmo §1.2 aponta
como as de disparo mais provável, são **5**, não 9. Reproduza com
`node research/tools/auditoria-peito/conferencia-curva.mjs`.

⚠️ **E o que o gate NÃO vê, que é maior do que o floor press:** as contagens acima e o
`R3` ("supino ≥ 22 séries/semana") contam só séries de TRABALHO. O bloco tem ainda **18
séries de aquecimento por semana sobre o peitoral, invariantes nas 16 semanas**, das quais
14 são de supino pausado — e a pausa de 1,0 s vale *"INCLUSIVE NOS 3 AQUECIMENTOS"* (§2.2
e a nota da linha). A exposição real do peitoral é de **42 a 46 séries por semana**, das
quais 27 a 31 pausadas no peito. Nenhum degrau desta tabela toca nas 18 de aquecimento.
...
não está funcionando"* `[R1 @01:30]` = **`V001-08`**, `[GERAL]` **`opiniao`**, e *"picos
isolados de dor um dia ou outro são normais… mas se os picos são frequentes, a dor platôa
ou piora, é preciso reduzir mais a carga"* `[R27 @05:53]` = **`V027-28`**, `[GERAL]`
`prescricao`.

⚠️ **A ÂNCORA DO DESVIO É `opiniao`, E ISSO PRECISA ESTAR ESCRITO.** Pela regra da casa,
só `modo: prescricao` vira treino; `opiniao` é contexto. A única claim que sustenta
"congela no 1º evento, sem recuo" é `V001-08`, que é **opinião**. A `V027-28`, que é
prescrição, diz o contrário para o caso repetido (*"reduza mais a carga"*), e é ela que
governa o 2º degrau. **Portanto o 1º degrau é DECISÃO DE DESENHO, não derivação da base** —
legítima, conservadora na direção certa, e agora declarada como o que é.

**O limiar de 2/10** é `[R1 @01:00]` = **`V001-06`**, `[GERAL]` `prescricao`.

⚠️ **RÓTULO CORRIGIDO, e ele era o único argumento escrito para a escolha do limiar.** A
versão anterior escrevia: *"o limiar de 2/10 é `[R1 @01:04]` `[GERAL]`; `[R79 @03:35]` dá
2–3/10 e é **[PESSOAL]**"*. Duas coisas estavam erradas. (a) Os dois endereços apontavam
para o bloco errado: `01:04` cai no bloco de `V001-06`/`V001-29` mas era usado TAMBÉM para
`V001-08`, que está 30 s adiante e tem `modo` diferente; e `03:35` cai no bloco de
`V079-30`/`V079-31`, que falam de MEDO e relesão, não de limiar. (b) A claim do 2–3/10 é
**`V079-34`** `[R79 @03:47]`, e ela é **`scope: GERAL`, `modo: prescricao`** — **não
`[PESSOAL]`**. O rótulo foi aplicado à mão contra o que está gravado na base.

**Consequência, e ela fica escrita:** descartar o 2–3/10 por ser "pessoal" **não tem base**
— `V079-34` é prescrição GERAL, mesmo tier e mesma classe de autoridade que `V001-06`. A
disputa real é entre duas prescrições GERAL que discordam, e escolher a mais apertada é
**preferência**, não derivação. O que de fato sustenta o 2/10 é outra coisa: `V138-19`
(2–4/10) é `opiniao` e por isso nunca esteve na disputa; e as condições de cautela
empilhadas — `V027-23` (*"lesões menores acabam sendo movidas mais do que deviam"*, condição
comum aos TRÊS limiares), `V079-39` (*"be cautious still"*), `V086-20` e `V086-22` — todas
empurram para o lado conservador.

⚠️ **E o 2/10 da base é ALVO; o 2/10 desta tabela é FREIO.** `V001-06` diz *"about a 2 out
of 10 level of pain is usually **good** for most people"* e `V079-34` diz *"a good amount
to **push at**"* — nas duas, 2/10 é o ponto onde se QUER estar. Aqui é o ponto onde se
congela. É o mesmo número com o sinal invertido, é uma escolha conservadora defensável
para um tecido com histórico, e **não é derivação de `V001-06`**: o número é emprestado e o
significado é trocado.

**Se o gate disparar entre S8 e S11, o bloco de 2 s é a PRIMEIRA coisa que sai.**

⚠️ **ESTA REGRA CONTRADIZ A CLAIM QUE O PRÓPRIO BLOCO INVOCA, e fica declarada.**
`V089-25` `[R89 @03:17]` (`GERAL`, `prescricao`) diz que pausas são o **contorno**:
*"a useful workaround is doing slow velocity variations, things like tempo work or pauses —
tendon strain is based off weight and speed we move it at"*. Por essa claim, o bloco de
2 s (4×3 a **65 % do TM**, RPE 6 — a linha mais leve e mais longe da falha entre todas as
pausadas do bloco, e existe em apenas 2 linhas de todo o programa) **reduz** strain. O pico
de tensão mora em outro lugar: o **back-off de D2**, 4×3 pausado a **82→86 % do TM a RPE
9,5**, em 13 das 16 semanas sem exceção — somado ao single, são 13 reps por semana em
comprimento máximo acima de 80 % do TM, da S4 à S16. Se o critério for tensão sobre o
tecido, a primeira coisa a sair é o back-off, não o bloco de 2 s.
⚠️ **Ressalva que não se omite:** `V089-25` está na mesma marca de `V089-24`, dentro do
bloco de **tendinite** — "workaround" é workaround de tendinite, e este atleta **não tem
esse diagnóstico**. A contradição fica registrada, não resolvida.

⚠️ **[PESSOAL]** o próprio Vena passou anos treinando através de dor pequena com tendinite
de tendão peitoral e *"ficou patinando sem progresso e sem melhora da dor"* `[R27 @05:21]`
= **`V027-25`**, `PESSOAL` `anedota` — é o modo de falha a evitar, e o rótulo `[PESSOAL]`
aqui está **correto**.
...
peitoral, porque o tendão do peitoral se insere perto do bíceps"* `[R95 @03:16]` =
**`V095-23`** (`GERAL`, **`mecanismo`**) + **`V095-24`** (`GERAL`, **`opiniao`**, é dela o
número 90 %).

⚠️ **ESTA REGRA É DECISÃO DE DESENHO, NÃO DERIVAÇÃO — declarado em 12/08/2026.** Ela é
operacional: decide em que gaveta o sintoma cai e, portanto, se o gate dispara. E está
ancorada em `mecanismo` + `opiniao`, **zero prescrição**. Procurei a prescrição que a
sustentasse e ela não existe: cruzando a base inteira, `topic: peito` ∩ (`dor` ∪ `lesao`)
dá **2 claims em 6.912**, e são exatamente `V027-25` (`PESSOAL`/`anedota`) e `V095-24`
(`GERAL`/`opiniao`) — **nenhuma prescrição**. A regra fica valendo porque errar para o lado
de registrar é o lado seguro num tecido com histórico de ruptura, mas ela é **escolha do
desenho** e não pode ser citada como se a base a mandasse.

⚠️ **E o transplante maior, que nenhuma onda tinha declarado:** como não há prescrição
alguma sobre peitoral + dor, **toda** instrução deste §1.2 (`V138-04`, `V138-18`, `V001-05`,
`V001-06`, `V079-34`, `V089-24/25`, `V108-27/30`) é claim de **lesão genérica transplantada
para o peitoral**. Isso é legítimo e é o melhor que a base tem — e é um transplante maior
do que o `PESSOAL → GERAL` que o documento inteiro vigia. Fica declarado aqui.
...
| Rosca na Polia | 0 | 3 | 8-12 | N/A | 9 | 1-1.5 MIN | Migrada de D3 [SPEC §1.3]. Bíceps tem crédito ZERO do SBD: todo volume é aditivo [design §10-B]. Entra por ESTÉTICA, que é razão suficiente e honesta. ⚠️ CORREÇÃO EXPLÍCITA: "bíceps forte protege o cotovelo no low bar" NÃO EXISTE na base — zero claims em 3.154; as soluções documentadas para cotovelo no agachamento são POSICIONAIS, e narrativa de "músculo fraco" é tratada como nocebo a evitar [R1 @06:38] [GERAL]. ⚠️ CONTRADIÇÃO DECLARADA: [R10 @02:08] prescreve 1 a 3 séries por músculo; [R145 @05:13] prescreve 8 a 15; e o que ele FAZ é outra coisa ainda — [R170 @03:34] [PESSOAL] 3 séries de rosca cerca de 5 dias por semana, [R154 @01:06] [PESSOAL] 4 séries de rosca todo dia, [R171 @07:23] [PESSOAL] rosca 5×/semana e meia polegada de braço a mais. Ele recomenda 1–3 e faz ~15. As 11 séries por semana deste programa são escolha da spec dentro dessa lacuna |
...
**POR QUE UM SIMULADO, E COM O QUÊ** — é precedente do canal e é **[PESSOAL]**: *"para o
mock meet ele dirige até uma academia que tem ANILHAS CALIBRADAS"* `[R7 @01:37]`
**[PESSOAL]** (`V007-06`, `narrativa`). ⚠️ **CITAÇÃO RETIRADA POR NÃO SE SUSTENTAR, e o
que sobra muda a força do argumento.** A versão anterior desta nota trazia uma segunda
procedência — *"sem competição marcada, considera fazer um mock meet com equipamento
específico de competição para ter um teste legítimo"*, endereçada a `R185 @04:38` (escrita
aqui **sem colchetes de propósito**: com colchetes ela voltaria a ser uma citação viva e
`check-enderecos.mjs` a cobraria de novo) — e ela **não existe
na base**: R185 não tem claim entre 04:04 e 05:05, e nenhuma claim do corpus inteiro
afirma isso. Não é erro de segundo: é conteúdo sem lastro. **Logo a procedência do canal
para este dia é UMA claim `PESSOAL`/`narrativa`, não duas** — precedente pessoal, nunca
prescrição. O simulado continua no bloco por decisão de desenho (§8 e a regra da 3ª
tentativa), e essa decisão passa a estar declarada como desenho em vez de emprestar
autoridade de uma citação que não resolve.

⚠️ **E o endereço da citação que ficou também estava errado, de um jeito que nenhuma
aritmética pega:** a versão anterior escrevia `R7 @01:23` (idem, sem colchetes), que **resolve** — só que para
`V007-05`, *"mesmo depois de falhar, ele acha que vai agachar mais na semana seguinte"*,
que não tem relação com anilhas calibradas. Endereço dentro da faixa apontando para o
bloco errado é a classe de defeito que `check-enderecos.mjs` **não** enxerga e que só a
leitura pega; está registrado no `RUNBOOK.md` §8.
...
| Agachamento Low Bar (Profundidade Legal) | 0 | 1 | 1 | 94.2% | 8.5 | 5 MIN | 2ª TENTATIVA — salto grande primeiro, salto menor depois: "opener bem fácil, depois um salto muito grande, depois um salto menor" [R21 @03:51] [GERAL, escola de Marcellus]. Joelhos para frente e para fora na saída [R169 @14:14] [GERAL] |
...
- ⚠️ **Contradição de mecanismo NÃO resolvida pela base:** `VÍDEO-BP` §3.4 diz que o
  ...
  **poupa** o tendão — `[R177 @02:39]`, `[R89 @03:14]`, `[R103]`, `[R170 @11:05]`). O bloco
