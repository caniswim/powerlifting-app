...
export function useSetCompletion(
  ...
  const saveEditSetFn = useCallback(() => {
    ...
    setEditingSet(null);
  }, [workout, editingSet, editWeight, editReps, editRPE, setWorkout]);
  ...
  const completeSet = useCallback(() => {
    ...
    if (allDone) {
      ...
      setSessionIndex(getSessionIndex() + 1);
    }
  ...
  }, [workout, activeExIdx, activeSetIdx, inputWeight, inputReps, inputRPE, setWorkout, setActiveExIdx, setActiveSetIdx, prefillInputs]);
